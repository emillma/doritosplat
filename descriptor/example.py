...
class ViT(nn.Module):
    def __init__(self, args):
        ...
        self.encoders = nn.ModuleList(
            [EncoderBlock(args) for _ in range(self.num_encoders)]
        )
...
def main():
    ...
    parser.add_argument("--num-heads", type=int, default=12, help="(default : 16)")
    parser.add_argument(
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-2, help="base learning rate (default : 0.01)"
    )
    parser.add_argument(
        "--weight-decay",
        type=int,
    ...
    transforms = Compose([Resize((args.img_size, args.img_size)), ToTensor()])
    train_data = torchvision.datasets.CIFAR10(
        root="./dataset", train=True, download=True, transform=transforms
    )
    valid_data = torchvision.datasets.CIFAR10(
        root="./dataset", train=False, download=True, transform=transforms
    )
    ...
    TrainEval(
        args, model, train_loader, valid_loader, optimizer, criterion, device
    ).train()
